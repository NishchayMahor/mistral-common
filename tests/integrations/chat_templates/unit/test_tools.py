...
class TestV2ToolResultParsing:
    ...
    @pytest.mark.parametrize(
        ("content", "expected"),
        [("4", '"content": 4'), ("3.14", '"content": 3.14')],
    )
    def test_numeric_tool_result(self, v2_template: str, content: str, expected: str) -> None:
        tool_name = "add" if content == "4" else "pi"
        user_content = "What is 2+2?" if content == "4" else "What is pi?"
        arguments = '{"a": 2, "b": 2}' if content == "4" else "{}"
        assistant_content = content if content == "4" else f"Pi is {content}"
...
class TestV3ToolResultParsing:
    ...
    def test_numeric_tool_result(self, v3_template: str, content: str, expected: str) -> None:
        tool_name = "add" if content == "4" else "pi"
        user_content = "What is 2+2?" if content == "4" else "What is pi?"
        arguments = '{"a": 2, "b": 2}' if content == "4" else "{}"
        assistant_content = content if content == "4" else "3.14"
